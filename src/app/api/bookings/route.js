...
export async function POST(req) {
    try {
        ...
        const [existing] = await pool.query(
            'SELECT id, status FROM bookings WHERE user_id = ? AND event_id = ?',
            [session.id, event_id]
        );

        if (existing.length > 0) {
            return NextResponse.json({
                error: 'You have already interacted with this event.',
                bookingId: existing[0].id, // Fixed: variable is 'existing'
        ...
        const [eventRows] = await pool.query(
            'SELECT capacity, booked FROM events WHERE id = ?',
            [event_id]
        );

        if (eventRows.length === 0) return NextResponse.json({ error: 'Event not found' }, { status: 404 });

        if (eventRows[0].booked >= eventRows[0].capacity) {
